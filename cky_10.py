...
class CKY:
    ...
    def buildIndices(self,productions):
        ...
        '''
        # Dividing given rules into unary ruls and binary rules before saving them in different defaultdicts separately.
        # And define defaultdicts unary to save unary rules, define defaultdicts binary to store unary roles
        # For example (X - X Y) in binary and (X - Y) in unary
    ...
    def maybeBuild(self, start, mid, end):
        ...
        for s1 in self.matrix[start][mid].labels():
            for s2 in self.matrix[mid][end].labels():
                print "hahahaha",s1,s2,start,mid,end
